...
import { Search, Filter, Star, Ban, Trash2, RefreshCw, Shield, ChevronDown, CheckCircle, Crown } from 'lucide-react';
...
export default function UsersPage() {
  ...
  const doAction = async (type: string, user: Profile) => {
    ...
    try {
      let updateData = {};
      if (type === 'grantPro') updateData = { pro_status: true };
      else if (type === 'revokePro') updateData = { pro_status: false };
      else if (type === 'makeOwner') updateData = { role: 'owner' };
      else if (type === 'makeAdmin') updateData = { role: 'admin' };
      else if (type === 'makeModerator') updateData = { role: 'moderator' };
      else if (type === 'makeUser') updateData = { role: 'user' };
      ...
      <div className="glass rounded-2xl overflow-x-auto shadow-sm">
        ...
        {loading ? (
          ...
          <div className="min-w-[700px]">
            {filtered.map((user) => (
              <div key={user.id} className={clsx("grid grid-cols-[auto_1fr_auto_auto_auto] gap-4 items-center px-6 py-4 border-b transition-colors", isDark ? "border-gray-800/30 hover:bg-white/[0.02]" : "border-slate-50 hover:bg-slate-100/50", user.is_banned && "opacity-70 bg-red-500/[0.02]")}>
                {/* Avatar */}
                ...
                <div className="flex items-center justify-end gap-1">
                  ...
                  <div className="relative group">
                    <button className="p-2.5 rounded-xl text-gray-400 hover:text-[#007AFF] hover:bg-[#007AFF]/10 transition-colors">
                      <Shield size={14} />
                    </button>
                    <div className={clsx("absolute bottom-full right-0 mb-2 w-32 rounded-xl border p-1 shadow-2xl opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all z-20", isDark ? "bg-gray-900 border-gray-800" : "bg-white border-slate-200")}>
                      <button onClick={() => setModal({ type: 'makeOwner', user })} className="w-full text-left px-3 py-1.5 text-[10px] font-bold rounded-lg hover:bg-yellow-500/10 text-yellow-500 transition-colors">Make Owner</button>
                      <button onClick={() => setModal({ type: 'makeAdmin', user })} className="w-full text-left px-3 py-1.5 text-[10px] font-bold rounded-lg hover:bg-[#007AFF]/10 text-[#007AFF] transition-colors">Make Admin</button>
                      <button onClick={() => setModal({ type: 'makeModerator', user })} className="w-full text-left px-3 py-1.5 text-[10px] font-bold rounded-lg hover:bg-purple-500/10 text-purple-500 transition-colors">Make Mod</button>
                      <button onClick={() => setModal({ type: 'makeUser', user })} className="w-full text-left px-3 py-1.5 text-[10px] font-bold rounded-lg hover:bg-gray-500/10 text-gray-500 transition-colors">Make User</button>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={clsx("glass rounded-[32px] p-8 w-full max-w-sm shadow-2xl border", isDark ? "border-white/10" : "border-slate-200")}>
            <div className={clsx("w-12 h-12 rounded-2xl flex items-center justify-center mb-6", 
              modal.type === 'banUser' ? 'bg-red-500/10 text-red-500' : 
              modal.type === 'makeOwner' ? 'bg-yellow-500/10 text-yellow-500' :
              'bg-[#007AFF]/10 text-[#007AFF]')}>
              {modal.type === 'banUser' ? <Ban size={24} /> : modal.type === 'makeOwner' ? <Crown size={24} /> : <Shield size={24} />}
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={clsx(
                  ...
                  modal.type === 'banUser' ? 'bg-red-600 hover:bg-red-500' : modal.type === 'makeOwner' ? 'bg-yellow-600 hover:bg-yellow-500' : 'bg-[#007AFF] hover:bg-[#0066dd]'
                )}
              >
